...
`carl pack` (subcommands `list` and `show <pack-id>`, both supporting `--json`) discovers and inspects instruction packs. Pack IDs are `<category>/<name>`, derived from the canonical path `.github/instructions/<category>/<name>.instructions.md`. Discovery merges bundled, repository-local, and `runtime.json`-selected sources, sorted by pack ID — filesystem enumeration order is never policy order. Metadata is schema-versioned (`schemaVersion: 1`); the discovered set is validated (malformed/duplicate IDs, invalid versions, unknown schema versions, missing or cyclic dependencies, invalid owned artefacts, contradictory state) before output. JSON errors are structured payloads on stderr with non-zero exit (via `cmdutil.ExitError` with `SuppressPrefix`). Pack selection is distinct from priority and override authority; only selection is modelled — `active` is currently derived from `selected`. It requires no network access and works outside an initialised repository (bundled packs only).
...
## Last updated
2026-07-23 by release notarisation automation update