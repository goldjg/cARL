...
- `sign-darwin` job runs on `macos-latest`; darwin signing/notarisation requires macOS tooling.
- ubuntu GoReleaser `release` job remains on `ubuntu-latest` for Linux/Windows builds, archives, checksums, Homebrew, and GitHub Release publication.
- GoReleaser uses `builder: prebuilt` for darwin, referencing signed binaries at `./prebuilt/darwin_<arch>/carl`.
...
- Temporary keychain must be deleted in an `always()` cleanup step in `sign-darwin`.
